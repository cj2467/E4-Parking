...
// Algorithm for removing noise and checking if it is a car or not
  // if first then both then second we know its a car or long object
  // cars typically pass within 10cm to 2m wehen entering lot from this side
  // using a nested if statement to enclose all 3 situations
  //high means the sensor saw something in the range on its last detection cycle
  // low means the sensor cannot see anything in the defined range
  //cars would have to be within 12 microseconds of each other for a 2 car to be counted as 1 car
  if (distanceClose > 10 and distanceClose < 200 and lastSensorStateClose == LOW) {
    lastSensorStateClose = HIGH;
    count++;
  }

  if (distanceClose > 200 or distanceClose <=0){//and lastSensorStateClose == LOW) {
...
//  }
//  
//  if (distanceClose > 200 ){//and lastSensorStateClose == LOW) {
//    lastSensorStateClose = LOW;
//  }
//  
////FAR Sensor tests
...
//  }
  ...
  // Resets Count if less than 0, cant be negative cars in the lot
  if (count < 0) {
       count = 0;
    }   
   // Resets Count if greater, for testing purposes
  if (count > 3) {
       count = 3;
    }   
